...
contract TabooMarketplace is Ownable, ReentrancyGuard {
    ...
    function _payout(
        ...
        if (auctionERC20Token == address(0)) {
            ...
            uint256 AmountTransfer = ((_amount * 10416666667) / 1e10);
            if (IERC20(auctionERC20Token).isExcluded(_recipient)) {
                AmountTransfer = _amount;
            }
            IERC20(auctionERC20Token).transfer(_recipient, AmountTransfer);
        }
    }
    ...
    function withdrawAuction(
        ...
        if (prevNftHighestBidder != address(0)) {
            if (_isERC20Auction(auctionERC20Token)) {
            uint256 AmountTransfer = ((prevNftHighestBid * 10416666667) / 1e10);
            if (IERC20(auctionERC20Token).isExcluded(prevNftHighestBidder)) {
                AmountTransfer = prevNftHighestBid;
            }
